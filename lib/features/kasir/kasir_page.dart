...
class _KasirPageState extends State<KasirPage>
  ...
  void initState() {
    ...
    _cartCurve =
        CurvedAnimation(parent: _cartAnim, curve: Curves.easeOutCubic);
  ...
  Future<void> _checkout(CartProvider cart) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text('Total: ${formatRupiah(cart.totalPrice)}',
    ...
    try {
      ...
      await context.read<TransactionProvider>().createTransaction(
            cashierId: user.id, total: total, items: items);
    ...
    return Column(children: [
      Expanded(
        child: ListView(
          ...
          children:
              _buildMenuWidgets(context, categories, grouped, cart, stockMap),
        ),
      ),
            ...
            child: Padding(
              ...
              child: Row(children: [
                Icon(Icons.shopping_bag_rounded,
                    color: ClayColors.primary, size: 20),
                const SizedBox(width: 8),
                const Text('Pesanan',
                    style: TextStyle(
                        fontWeight: FontWeight.bold, fontSize: 15)),
